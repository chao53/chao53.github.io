...
**挑战：** 如何从抖动、噪声丰富、甚至偶尔完全错误的 MediaPipe 数据中，提取出平滑、自然且符合生物力学的人体动作？同时如何避免“过度滤波”带来的响应迟滞？

**解决方案：**

我们设计了一套多层次、逐步深入的数据净化管道。该管道从最粗粒度的“可信度”判断开始，到最细微的动画平滑结束，确保每一帧输出的姿态都是稳定且合理的。

#### 1. 置信度门槛（Confidence Gating）

**原理：** MediaPipe 在进行推断时，会为检测到的每一只手提供一个**置信度分数（0.0 - 1.0）**。当手部姿态清晰时，分数很高（>0.9）；当手掌正对/背对镜头导致姿态模糊时，分数会显著下降。我们利用这个分数作为数据质量的**第一道关卡**。

- **实现方式：**
  - **数据采集端（Python）**：将 MediaPipe 输出的置信度分数与 landmark 坐标、左右手标签一同打包成 JSON，完整地发送给 Unreal Engine。
  - **数据接收端（C++ AnimNode）**：在动画节点 FAnimNode_HandPoseFromLandmarks 中，我们引入了一个可配置的 ConfidenceThreshold（置信度阈值）参数。
  - **过滤逻辑：** 在 Evaluate_AnyThread 函数的**最开始**，我们会检查传入的手部数据的置信度。如果分数**低于**设定的阈值，我们会认为这帧数据是**不可靠的“坏数据”**。节点将**完全丢弃**这帧数据，并直接输出上游传入的基础姿态（BasePose），使模型手部恢复到默认或由其他动画驱动的姿态。
- **优势：**
...
在数据通过了置信度门槛，被确认为“可信”之后，我们对其进行双重平滑处理，以消除高频抖动。
...
为了让模型的动作更符合真实人体，我们对计算出的旋转施加了基于解剖结构的约束。

- **防止手指反向弯曲：**
  - MediaPipe 并不会主动判断手指弯曲的方向。
  - **[关键突破]** 在第二与第三节指关节（PIP 和 DIP）上，我们实现了基于**旋转分解与角度限制**的约束。
  - **具体做法：** 将指关节的旋转增量分解为“摆动（Swing）”和“扭转（Twist）”两部分。我们丢弃“扭转”，然后计算出“摆动”的**带符号角度**，并使用 FMath::Clamp 将其夹紧在一个符合生理的范围内（如 [-5°, 90°]），从而根除了手指“向后翻”的错误。
...
**挑战：** 如何让系统超越简单的姿态同步，真正“理解”用户意图？如何训练一个能识别我们自定义的、独一无二的手势（如数字0-9、比心等）的高性能模型？

**解决方案：** 我们采用了一套完整的、端到端的机器学习工作流，利用 **Google Colab** 的云端计算能力和 **MediaPipe Model Maker** 的迁移学习技术，实现了从数据采集到模型部署的全过程。

#### 1. 数据集工程化：高质量数据的采集与处理

我们深刻理解“数据决定模型上限”的原则，因此在数据准备阶段投入了大量精力，确保了数据集的**多样性**和**质量**。
...
- **构建多样化数据集**:
  ...
  - 我们为**数字0到9**、**点赞**、**比心**等十余种自定义手势，以及一个至关重要的**none（非手势）**类别，分别录制了视频。
  - 采集过程充分考虑了多样性，涵盖了**不同的人物、角度、光照和背景**，以增强模型的泛化能力。
  - 特别是 none 类别，我们采集了大量手部在移动、放松、以及手势切换过程中的“中间态”图片，以训练模型在非交互状态下保持“沉默”，避免误识别。

#### 2. 云端训练：利用 MediaPipe Model Maker 实现高效迁移学习

**挑战：** 在本地配置复杂的深度学习环境（CUDA, cuDNN）既耗时又容易出错，且对硬件要求高。

- **解决方案：** 我们将整个训练流程迁移到了 **Google Colab** 云端平台。

  - **环境零配置**: 直接利用 Colab 预置的、带免费 GPU 加速的 Python 环境，无需任何本地安装。

  - **稳定的数据流**: 为了解决 Colab 上传大文件时容易中断的问题，我们采用了**“Google Drive 中转”**的方案：先将打包好的数据集 .zip 文件上传到 Google Drive，然后在 Colab 中挂载 Drive，并高速复制到本地环境再解压。这套流程被证明是极其稳定和高效的。

  - **迁移学习**: 我们没有从零开始训练模型。**MediaPipe Model Maker** 在后台自动加载了 Google 预训练的、强大的**手部特征提取基础模型**，我们只需在其之上，用自己的数据集来微调（Fine-tuning）一个全新的、为我们自定义手势量身定做的分类头。这使得训练过程不仅速度飞快（通常在十几分钟内完成），而且模型起点高，准确率非常有保障。

    ![GoogleTrain](images\GoogleTrain.jpg)

#### 3. 端到端部署与交互

训练完成后，我们将定制化的 .task 模型集成回我们的系统中，打通了从识别到交互的最后一环。

- **Python 端升级**:
  - 我们将原有的 Hands 模型完全替换为新的、功能更强大的 **GestureRecognizer**。它不仅能提供与之前完全一致的坐标、左右手标签和置信度数据，还额外输出了我们刚刚训练的**自定义手势标签**（如 "Thumb_Up", "Heart", "One", "Two" 等）。
  - 所有这些信息被实时打包成 JSON，通过 UDP 发送给 UE。
- **UE 端接收与响应**:
  - C++ AX_HandDataReceiverActor 负责解析新增的 Gesture 字段。
  - 在动画蓝图或行为树中，我们可以轻松地获取到这个手势字符串，并用一个简单的 Switch on String 节点来分发逻辑。
  - **[最终效果]** 这使得我们可以实现丰富的即时交互：做出“一”的手势可以切换武器，做出“比心”手势可以触发一个爱心粒子特效，做出“点赞”可以获得分数。系统不再仅仅是“镜子”，而是成为了一个能与用户进行智能沟通的“伙伴”。
